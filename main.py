...
async def _get_session_with_auth(session_id: str, current_user: Dict) -> Dict:
    """세션을 조회하고 권한을 확인합니다."""
    session = SESSIONS.get(session_id)
    ...
    if owner and owner != current_user.get("id"):
        raise HTTPException(status_code=403, detail="Not authorised for this session")
    
    return session

async def _call_k8s_api(session_id: str, path: str, method: str = "GET", params: Optional[Dict] = None) -> Dict:
    """세션의 k8s-api-server에 요청을 보냅니다."""
    session = SESSIONS.get(session_id)
    if not session:
        raise HTTPException(status_code=404, detail="Session not found")
    ...
    try:
        ...
        if response.status_code != 200:
            logger.warning(
                "k8s-api request failed for session %s. Status: %s, Details: %s",
                session_id,
                response.status_code,
                response.text,
            )
            raise HTTPException(
                status_code=502,
                detail=f"k8s-api-server returned status {response.status_code}"
            )
        
        return response.json()
    except requests.exceptions.RequestException as e:
        logger.error(f"Failed to call k8s-api for session {session_id}: {e}")
        raise HTTPException(status_code=502, detail="Failed to reach k8s-api-server")

@app.get("/session/{session_id}/pods")
async def get_session_pods(session_id: str, current_user: Dict = Depends(get_current_user)):
    """특정 세션의 pods를 조회합니다."""
    await _get_session_with_auth(session_id, current_user)
    
    try:
        result = await _call_k8s_api(session_id, "api/v1/pods")
        return result
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error fetching pods for session {session_id}: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")

@app.get("/session/{session_id}/nodes")
async def get_session_nodes(session_id: str, current_user: Dict = Depends(get_current_user)):
    """특정 세션의 nodes를 조회합니다."""
    await _get_session_with_auth(session_id, current_user)
    
    try:
        result = await _call_k8s_api(session_id, "api/v1/nodes")
        return result
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error fetching nodes for session {session_id}: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
